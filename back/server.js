...
app.delete("/review/:id", verifyAccessToken, deleteReview); // 리뷰 삭제 API
...
app.delete("/notification/:id", verifyAccessToken, deleteNotification); // 알림 삭제 API

// 댓글 관련 API
app.get("/comment/:id", getCommentById); // 특정 댓글 조회 API
app.post("/comment/:id", verifyAccessToken, addComment); // 리뷰 댓글 등록 API
app.delete("/comment/:id", verifyAccessToken, deleteComment); // 리뷰 댓글 삭제 API

// 추천 관련 API
app.patch("/like/:id", verifyAccessToken, addLike); // 리뷰 추천 API
app.get("/like/:id", verifyAccessToken, getUserLiked); // 사용자의 리뷰 추천 여부 조회 API
app.get("/like/:id/count", getLikeCount); // 리뷰 추천 수 조회 API
app.patch("/unlike/:id", verifyAccessToken, unLike); // 리뷰 추천 취소 API
